...
jQuery(document).ready(function($) {
    ...
    if( $("#header-wrapper .sticky-header").length > 0 ) {
        ...
        $sticky_header_cloned
            .find('.wdt-header-icons-list-item.search-item.search-overlay')
            .each(function() {
                var $searchContainer = $(this).find('.wdt-search-form-container');
                if ($searchContainer.length) {

                    // FIX: Clone instead of removing completely
                    var $clonedForm = $searchContainer.clone();
                    $searchContainer.remove();
                    $(this).append($clonedForm);
                }
            });
